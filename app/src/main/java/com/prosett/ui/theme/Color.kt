package com.prosett.ui.theme

import androidx.compose.ui.graphics.Color

// Light Palette
val PrimaryLight = Color(0xFF0284C7)
val OnPrimaryLight = Color(0xFFFFFFFF)
val PrimaryContainerLight = Color(0xFFE0F2FE)
val OnPrimaryContainerLight = Color(0xFF0369A1)

val SecondaryLight = Color(0xFF0F766E)
val OnSecondaryLight = Color(0xFFFFFFFF)
val SecondaryContainerLight = Color(0xFFCCFBF1)
val OnSecondaryContainerLight = Color(0xFF115E59)

val TertiaryLight = Color(0xFF854D0E)
val OnTertiaryLight = Color(0xFFFFFFFF)
val TertiaryContainerLight = Color(0xFFFEF3C7)
val OnTertiaryContainerLight = Color(0xFF78350F)

val BackgroundLight = Color(0xFFF8FAFC)
val OnBackgroundLight = Color(0xFF0F172A)
val SurfaceLight = Color(0xFFFFFFFF)
val OnSurfaceLight = Color(0xFF0F172A)
val SurfaceVariantLight = Color(0xFFF1F5F9)
val OnSurfaceVariantLight = Color(0xFF475569)

// Dark Palette
val PrimaryDark = Color(0xFF38BDF8)
val OnPrimaryDark = Color(0xFF082F49)
val PrimaryContainerDark = Color(0xFF075985)
val OnPrimaryContainerDark = Color(0xFFE0F2FE)

val SecondaryDark = Color(0xFF2DD4BF)
val OnSecondaryDark = Color(0xFF042F2E)
val SecondaryContainerDark = Color(0xFF115E59)
val OnSecondaryContainerDark = Color(0xFFCCFBF1)

val TertiaryDark = Color(0xFFFBBF24)
val OnTertiaryDark = Color(0xFF451A03)
val TertiaryContainerDark = Color(0xFF78350F)
val OnTertiaryContainerDark = Color(0xFFFEF3C7)

val BackgroundDark = Color(0xFF0B1120)
val OnBackgroundDark = Color(0xFFF1F5F9)
val SurfaceDark = Color(0xFF131D31)
val OnSurfaceDark = Color(0xFFF8FAFC)
val SurfaceVariantDark = Color(0xFF1E293B)
val OnSurfaceVariantDark = Color(0xFF94A3B8)

// Neutral Dark (Material Standard)
val BackgroundNeutralDark = Color(0xFF121212)
val SurfaceNeutralDark = Color(0xFF1E1E1E)
val SurfaceVariantNeutralDark = Color(0xFF2A2A2A)

// Tinted Dark (Blue/Plum)
val BackgroundTintedDark = Color(0xFF0B132B)
val SurfaceTintedDark = Color(0xFF161B22)
val SurfaceVariantTintedDark = Color(0xFF1A1B26)

// Status Colors
val StatusAllowed = Color(0xFF10B981)
val StatusBlocked = Color(0xFFEF4444)
val StatusWarning = Color(0xFFF59E0B)
val StatusRestricted = Color(0xFFA855F7)
val StatusActiveCyan = Color(0xFF06B6D4)
